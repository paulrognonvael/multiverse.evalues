...
summar.lin <- read_csv("~/GitHub/multiverse.evalues/simulations/linreg/summaries.all.csv", 
                       col_types = cols(sim = col_skip(), n = col_integer()))

for(i in 1:length(beta)){
  betai = colnames(summar.lin)[i]
  summar.lin %>% select(all_of(c(betai,'n','type'))) %>% 
    filter(type%in%c('mixt','calib1','calib3','soft')) %>% 
    ggplot() + geom_line(aes(x=n,y=.data[[betai]], linetype = type))+
    geom_point(aes(x=n,y=.data[[betai]], shape = type)) +
    theme_light(base_size = 10)+
    ...
    ylab('log e-value')
  #ggtitle(TeX(sprintf(r"($\beta_i=%s$)",beta[i])))
  ggsave(paste0('linreg/sim.linreg.paper.beta',beta[i],'.png'),width =60, height = 60, units='mm')
  ...
  summar.lin %>% select(all_of(c(betai,'n','type'))) %>% 
    ...
    ylab('log e-value or Bayes factor')
  # ggtitle(TeX(sprintf(r"($\beta_i=%s$)",beta[i])))
  ggsave(paste0('linreg/sim.linreg.allev.beta',beta[i],'.png'),width =60, height = 60, units='mm')
